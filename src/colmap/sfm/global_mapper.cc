...
// Returns the number of registered images that have a position prior.
size_t CountImagesWithPriorPosition(
  ...
  for (const auto& prior : pose_priors) {
    ...
    if (reconstruction.Image(image_id).HasPose()) ++count;
  }
  return count;
}
...
bool RunBundleAdjustment(const BundleAdjustmentOptions& options,
  ...
  const bool use_prior_ba =
      use_prior_position &&
      CountImagesWithPriorPosition(reconstruction, pose_priors) >= 3;

  if (use_prior_ba) {
    THROW_CHECK(options.backend == BundleAdjustmentBackend::CERES)
        << "Pose-prior bundle adjustment requires the Ceres backend; "
           "set --GlobalMapper.ba_backend ceres";
    // Do NOT fix gauge: the prior BA aligns and manages it internally.
...
PosePriorBundleAdjustmentOptions GlobalMapperOptions::PriorBundleAdjustment()
    const {
  PosePriorBundleAdjustmentOptions opts;
  if (use_robust_loss_on_prior_position) {
    opts.ceres->prior_position_loss_function_type =
        CeresBundleAdjustmentOptions::LossFunctionType::CAUCHY;
  }
  opts.ceres->prior_position_loss_scale = prior_position_loss_scale;
  opts.prior_position_fallback_stddev = prior_position_fallback_stddev;
  opts.use_prior_rotation = use_prior_rotation;
  opts.prior_rotation_fallback_stddev_deg = prior_rotation_fallback_stddev_deg;
  if (random_seed >= 0) {
  ...
  for (int ite = 0; ite < num_iterations; ite++) {
    ...
    if (!skip_fixed_rotation_stage) {
      ...
      if (!RunBundleAdjustment(
              opts_position_only, *reconstruction_,
              use_prior_position, prior_options, pose_priors)) {
        return false;
      }
